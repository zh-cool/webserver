...
#if MG_ENABLE_TCPIP && !defined(MG_ENABLE_DRIVER_STM32F) && !defined(MG_ENABLE_DRIVER_STM32H) && !defined(MG_ENABLE_DRIVER_STM32N)
...
#if (MG_ARCH == MG_ARCH_CMSIS_RTOS1 || MG_ARCH == MG_ARCH_CMSIS_RTOS2) && !defined MG_ENABLE_RL && (!defined(MG_ENABLE_LWIP) || !MG_ENABLE_LWIP) &&                                \
...
#define MG_SOCK_PENDING(errcode) (((errcode) < 0) && (WSAGetLastError() == WSAEINTR || WSAGetLastError() == WSAEINPROGRESS || WSAGetLastError() == WSAEWOULDBLOCK))
...
#define MG_SOCK_PENDING(errcode)                                                                                                                                                   \
	((errcode) == -pdFREERTOS_ERRNO_EWOULDBLOCK || (errcode) == -pdFREERTOS_ERRNO_EISCONN || (errcode) == -pdFREERTOS_ERRNO_EINPROGRESS ||                                     \
...
#define MG_SOCK_PENDING(errcode) ((errcode) == BSD_EWOULDBLOCK || (errcode) == BSD_EALREADY || (errcode) == BSD_EINPROGRESS)
...
#define MG_U32(a, b, c, d) (((uint32_t)((a)&255) << 24) | ((uint32_t)((b)&255) << 16) | ((uint32_t)((c)&255) << 8) | (uint32_t)((d)&255))

#define MG_IPV4(a, b, c, d) mg_htonl(MG_U32(a, b, c, d))

#define MG_IPV6(a, b, c, d, e, f, g, h)                                                                                                                                            \
	{                                                                                                                                                                          \
		(uint8_t)((a) >> 8), (uint8_t)(a), (uint8_t)((b) >> 8), (uint8_t)(b), (uint8_t)((c) >> 8), (uint8_t)(c), (uint8_t)((d) >> 8), (uint8_t)(d), (uint8_t)((e) >> 8),   \
			(uint8_t)(e), (uint8_t)((f) >> 8), (uint8_t)(f), (uint8_t)((g) >> 8), (uint8_t)(g), (uint8_t)((h) >> 8), (uint8_t)(h)                                      \
	}
...
#define MG_LOAD_BE32(p) ((uint32_t)(((uint32_t)MG_U8P(p)[0] << 24U) | ((uint32_t)MG_U8P(p)[1] << 16U) | ((uint32_t)MG_U8P(p)[2] << 8U) | MG_U8P(p)[3]))
#define MG_LOAD_BE64(p)                                                                                                                                                            \
	((uint64_t)(((uint64_t)MG_U8P(p)[0] << 56U) | ((uint64_t)MG_U8P(p)[1] << 48U) | ((uint64_t)MG_U8P(p)[2] << 40U) | ((uint64_t)MG_U8P(p)[3] << 32U) |                        \
		    ((uint64_t)MG_U8P(p)[4] << 24U) | ((uint64_t)MG_U8P(p)[5] << 16U) | ((uint64_t)MG_U8P(p)[6] << 8U) | MG_U8P(p)[7]))
...
struct mg_connection *mg_connect_svc(struct mg_mgr *mgr, const char *url, mg_event_handler_t fn, void *fn_data, mg_event_handler_t pfn, void *pfn_data);
...
//
//
int mg_aes_gcm_encrypt(unsigned char *output, const unsigned char *input, size_t input_length, const unsigned char *key, const size_t key_len, const unsigned char *iv,
		       const size_t iv_len, unsigned char *aead, size_t aead_len, unsigned char *tag, const size_t tag_len);

int mg_aes_gcm_decrypt(unsigned char *output, const unsigned char *input, size_t input_length, const unsigned char *key, const size_t key_len, const unsigned char *iv,
		       const size_t iv_len, unsigned char *aead, size_t aead_len, const unsigned char *tag, const size_t tag_len);
...
Returns 1 if the signature generated successfully, 0 if an error occurred.
*/
int mg_uecc_sign_deterministic(const uint8_t *private_key, const uint8_t *message_hash, unsigned hash_size, const MG_UECC_HashContext *hash_context, uint8_t *signature,
			       MG_UECC_Curve curve);
...
mg_uecc_word_t mg_uecc_vli_add(mg_uecc_word_t *result, const mg_uecc_word_t *left, const mg_uecc_word_t *right, wordcount_t num_words);

/* Computes result = left - right, returning borrow. Can modify in place. */
mg_uecc_word_t mg_uecc_vli_sub(mg_uecc_word_t *result, const mg_uecc_word_t *left, const mg_uecc_word_t *right, wordcount_t num_words);
...
void mg_uecc_vli_modMult_fast(mg_uecc_word_t *result, const mg_uecc_word_t *left, const mg_uecc_word_t *right, MG_UECC_Curve curve);
...
*/
PORTABLE_8439_DECL size_t mg_chacha20_poly1305_encrypt(uint8_t *restrict cipher_text, const uint8_t key[RFC_8439_KEY_SIZE], const uint8_t nonce[RFC_8439_NONCE_SIZE],
						       const uint8_t *restrict ad, size_t ad_size, const uint8_t *restrict plain_text, size_t plain_text_size);
...
*/
PORTABLE_8439_DECL size_t mg_chacha20_poly1305_decrypt(uint8_t *restrict plain_text, const uint8_t key[RFC_8439_KEY_SIZE], const uint8_t nonce[RFC_8439_NONCE_SIZE],
						       const uint8_t *restrict ad, size_t ad_size, const uint8_t *restrict cipher_text, size_t cipher_text_size);
#if defined(__cplusplus)
}
#endif
#endif
...
int mg_rsa_crt_sign(const uint8_t *em, size_t em_len, const uint8_t *dP, size_t dP_len, const uint8_t *dQ, size_t dQ_len, const uint8_t *p, size_t p_len, const uint8_t *q,
		    size_t q_len, const uint8_t *qInv, size_t qInv_len, uint8_t *signature, size_t sig_len);
#endif // TLS_RSA_H
...
enum { MQTT_PROP_TYPE_BYTE, MQTT_PROP_TYPE_STRING, MQTT_PROP_TYPE_STRING_PAIR, MQTT_PROP_TYPE_BINARY_DATA, MQTT_PROP_TYPE_VARIABLE_INT, MQTT_PROP_TYPE_INT, MQTT_PROP_TYPE_SHORT };
...
struct mg_connection *mg_modbus_listen(struct mg_mgr *mgr, const char *url, mg_event_handler_t fn, void *fn_data);
...
void mg_ota_url_check(struct mg_mgr *mgr, const char *current_version, const char *metadata_url, void (*fn)(const char *status));
...
enum mg_l2proto { MG_TCPIP_L2PROTO_IPV4 = 0, MG_TCPIP_L2PROTO_IPV6, MG_TCPIP_L2PROTO_ARP, MG_TCPIP_L2PROTO_PPPoE_DISC, MG_TCPIP_L2PROTO_PPPoE_SESS };
...
#if MG_ENABLE_TCPIP && ((defined(MG_ENABLE_DRIVER_CYW) && MG_ENABLE_DRIVER_CYW) || (defined(MG_ENABLE_DRIVER_CYW_SDIO) && MG_ENABLE_DRIVER_CYW_SDIO))
...
#if MG_ENABLE_TCPIP && (defined(MG_ENABLE_DRIVER_IMXRT10) && MG_ENABLE_DRIVER_IMXRT10) || (defined(MG_ENABLE_DRIVER_IMXRT11) && MG_ENABLE_DRIVER_IMXRT11) ||                       \
...
#if MG_ENABLE_TCPIP && (defined(MG_ENABLE_DRIVER_RA6) && MG_ENABLE_DRIVER_RA6) || (defined(MG_ENABLE_DRIVER_RA8) && MG_ENABLE_DRIVER_RA8)
